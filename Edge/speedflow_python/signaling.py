#!/usr/bin/env python3
"""
Edge/speedflow_python/signaling.py — WebRTC Signaling Server (Embedded per-Peer).

Mỗi Edge Node chạy signaling server riêng (embedded coroutine trong asyncio event loop).
Grid Monitor kết nối trực tiếp đến signaling của từng Jetson.

Kiến trúc:
  - edges: dict[edge_id → websocket] — Lưu kết nối WebSocket từ pipeline (role=edge)
  - clients: list[websocket]         — Lưu kết nối từ Browser (role=browser)
  - Khi Browser gửi {"action":"view","target_edge":"jetson_A"}, Server forward SDP Offer
    đến đúng Edge pipeline và chuyển tiếp Answer ngược lại.

Có thể chạy standalone hoặc embedded:
  - Standalone: python3 signaling.py
  - Embedded:   asyncio.create_task(start_embedded(host, port))
"""
...
# Paths
# ---------------------------------------------------------------------------
SCRIPT_DIR = Path(__file__).parent / "grid_monitor"
...
async def _handle_edge(ws: web.WebSocketResponse, edge_id: str) -> None:
    ...
    edges[edge_id] = {"ws": ws, "registered_at": asyncio.get_running_loop().time()}
...
# ---------------------------------------------------------------------------
# Embedded Entry Point (chạy trong asyncio event loop của Edge process)
# ---------------------------------------------------------------------------
...
async def start_embedded(
    ...
    """
    Khởi động signaling server dưới dạng background coroutine.
    ...
    try:
        await site.start()
        nonlocal_logger.info("[Signaling] Embedded server running on %s:%d", host, port)
    except Exception as exc:
        nonlocal_logger.error("[Signaling] Failed to start embedded server: %s", exc)


# ---------------------------------------------------------------------------
# Standalone Entry Point (giữ lại để debug)
# ---------------------------------------------------------------------------
...
if __name__ == "__main__":
    ...
    from speedflow_python.settings import SIGNALING_HOST as HOST, SIGNALING_PORT as PORT
    ...
    logger.info("=" * 55)
    logger.info("  WebRTC Signaling Server (Standalone)")
